...
let servicios = [];
let preguntas = [
  {key:'empresa', q:'¿Cómo se llama tu empresa?', type:'text', placeholder:'Ej: Soluciones Andinas S.A.S. - software a la medida'},
  {key:'tam', q:'Tamaño de la empresa', type:'choice', opts:[
    {l:'Micro (1-10)',v:'Micro',pts:5},{l:'Pequeña (11-50)',v:'Pequeña',pts:10},{l:'Mediana (51-200)',v:'Mediana',pts:15},{l:'Grande (+200)',v:'Grande',pts:15}
  ]},
  {key:'sector', q:'¿Qué le vendes al Estado?', type:'choice', opts:[
    {l:'Servicios / Consultoría',v:'Servicios',pts:10},{l:'Tecnología / Software',v:'Tecnología',pts:10},{l:'Obra / Infraestructura',v:'Obra',pts:10},{l:'Bienes / Suministros',v:'Bienes',pts:10},{l:'Aún no lo tengo claro',v:'Indefinido',pts:0}
  ]},
  {key:'exp', q:'Experiencia en contratación pública', type:'choice', opts:[
    {l:'Nunca he participado',v:'Nunca',pts:0},{l:'Me he registrado pero nunca oferté',v:'Registrado',pts:10},{l:'Oferté 1-3 veces (sin ganar)',v:'Ofertó',pts:20},{l:'Ya he ganado contratos',v:'Ganador',pts:35}
  ]},
  {key:'rup', q:'¿RUP vigente?', type:'choice', opts:[
    {l:'No tengo / vencido',v:'No',pts:0},{l:'En trámite',v:'Trámite',pts:10},{l:'Sí, vigente',v:'Sí',pts:20}
  ]},
  {key:'secop', q:'Manejo de SECOP II', type:'choice', opts:[
    {l:'No lo conozco',v:'Nulo',pts:0},{l:'Básico (entro y busco)',v:'Básico',pts:10},{l:'Intermedio (he ofertado)',v:'Intermedio',pts:20},{l:'Avanzado (gestiono todo)',v:'Avanzado',pts:30}
  ]},
  {key:'equipo', q:'¿Equipo dedicado a licitaciones?', type:'choice', opts:[
    {l:'Nadie / solo yo',v:'Nadie',pts:0},{l:'1 persona parcial',v:'Parcial',pts:10},{l:'Equipo 2-3 personas',v:'Equipo',pts:20},{l:'Área estructurada',v:'Área',pts:25}
  ]},
  {key:'objetivo', q:'Objetivo principal a 6 meses', type:'choice', opts:[
    {l:'Ganar primer contrato',v:'Primer contrato',pts:5},{l:'Aumentar contratos actuales',v:'Escalar',pts:10},{l:'Diversificar entidades',v:'Diversificar',pts:10},{l:'Estructurar área de contratación',v:'Estructurar',pts:10}
  ]}
];
...
function addMsg(txt, who='bot'){
  const c=el('chatBody');
  if(!c) return;
  const d=document.createElement('div'); d.className='msg '+who; d.innerHTML=txt;
  c.appendChild(d); c.scrollTop=c.scrollHeight;
}
function addTyping(){
  const c=el('chatBody'); if(!c) return;
  const d=document.createElement('div'); d.className='msg bot'; d.id='typing'; d.innerHTML='<em>● ● ● escribiendo...</em>';
  c.appendChild(d); c.scrollTop=c.scrollHeight;
}
...
function updateRadarMini(){
  try{
    ...
    if(filled<2) return;
    const wrap=el('radarWrap'); if(wrap) wrap.style.display='block';
    const ctx = el('radarMini');
    if(!ctx) return;
    const data = [dims.habilitacion, dims.secop, dims.experiencia, dims.capacidad];
    if(radarMiniChart) { try{ radarMiniChart.destroy(); }catch(e){} radarMiniChart=null; }
    radarMiniChart = new Chart(ctx, {
      type:'radar',
      data:{ labels:['Habilitación','SECOP','Experiencia','Capacidad'], datasets:[{label:'Madurez', data, backgroundColor:'rgba(0,201,167,.18)', borderColor:'#00c9a7', pointBackgroundColor:'#0f1f3c'}]},
      options:{ scales:{r:{min:0,max:100,ticks:{stepSize:25, display:false}, pointLabels:{font:{size:9}}}}, plugins:{legend:{display:false}}, animation:false, responsive:true }
    });
  }catch(e){ console.log('radarMini error',e); }
}

function renderQuestion(){
  if(idx>=preguntas.length){ finish(); return; }
  ...
  setTimeout(()=>{
    ...
    const optBox=el('chatOptions'); if(!optBox) return; optBox.innerHTML='';
    if(p.type==='choice'){
      p.opts.forEach(o=>{
        const b=document.createElement('button'); b.className='opt-btn'; b.textContent=o.l;
    ...
    const row=el('chatInputRow'); if(row) row.style.display='flex';
...
function answer(key,val,pts){
  if(chatLocked) return; chatLocked=true;
  respuestas[key]=val;
...
function sendFreeText(){
  ...
  const inp=el('freeInput'); if(!inp) return;
  const v=inp.value.trim(); if(!v) return;
  if(v.length<2){ addMsg('Escribe al menos 2 caracteres','bot'); return; }
  const p=preguntas[idx];
  if(!p){ addMsg(v,'user'); inp.value=''; return; }
  if(p.type==='text'){
  ...
  try{
    const inf=nlp(v);
    if(p.key==='sector' && inf.sector){ mapped=inf.sector; pts=10; }
    else if(p.key==='rup' && inf.rup){ mapped=inf.rup; pts= inf.rup==="Sí"?20: inf.rup==="Trámite"?10:0; }
    else if(p.key==='secop' && inf.secop){ mapped=inf.secop; pts= inf.secop==="Avanzado"?30:inf.secop==="Intermedio"?20:inf.secop==="Básico"?10:0; }
    else if(p.key==='exp' && inf.exp){ mapped=inf.exp; pts= inf.exp==="Ganador"?35:inf.exp==="Ofertó"?20:inf.exp==="Registrado"?10:0; }
    else if(p.key==='equipo' && inf.equipo){ mapped=inf.equipo; pts= inf.equipo==="Área"?25:inf.equipo==="Equipo"?20:inf.equipo==="Parcial"?10:0; }
    else if(p.key==='tam'){
      if(lower.match(/micro|1-10/)) {mapped="Micro"; pts=5}
      else if(lower.match(/peque|11-50/)) {mapped="Pequeña"; pts=10}
      else if(lower.match(/mediana|50-200/)) {mapped="Mediana"; pts=15}
      else if(lower.match(/grande|\+200/)) {mapped="Grande"; pts=15}
    }
    else if(p.key==='objetivo' && inf.objetivo){ mapped=inf.objetivo; pts=10; }
  }catch(e){}
  if(mapped){
    addMsg(v,'user');
    addMsg(`<em>Entendido como: <strong>${mapped}</strong> (IA interpretó tu texto libre)</em>`);
    respuestas[p.key]=mapped; score+=pts;
    idx++; chatLocked=false;
    inp.value='';
    setTimeout(renderQuestion,650);
    return;
  }
  ...
  respuestas[p.key]=v; score+=8; idx++; chatLocked=false; inp.value=''; setTimeout(renderQuestion,650);
}

function resetState(){
  idx=0; score=0; respuestas={}; chatLocked=false; modoLibrePendiente=false;
  const body=el('chatBody'); if(body) body.innerHTML='';
  const res=el('resultados'); if(res) res.style.display='none';
  const row=el('chatInputRow'); if(row) row.style.display='none';
  const card=el('perfilCard');
  if(card){
    const empty=card.querySelector('.perfil-empty');
    const data=el('perfilData');
  ...
  const wrap=el('radarWrap'); if(wrap) wrap.style.display='none';
  const guia=el('guiaLibre'); if(guia) guia.style.display='none';
  const bar=el('progressBar'); if(bar) bar.style.width='0%';
  const txt=el('progressTxt'); if(txt) txt.textContent='Reiniciado • Listo para iniciar';
  ['pEmpresa','pSector','pTam','pExp','pRup','pSecop','pEquipo','pObj'].forEach(id=>{const e=el(id); if(e) e.textContent='-';});
  if(radarMiniChart){ try{radarMiniChart.destroy();}catch(e){} radarMiniChart=null; }
  if(radarFullChart){ try{radarFullChart.destroy();}catch(e){} radarFullChart=null; }
}
...
function startAgent(){
  ...
  renderHistorial(); updateEvoChart();
}
...
function modoLibre(){
  ...
  const row=el('chatInputRow'); if(row) row.style.display='flex';
  const inp=el('freeInput'); if(inp){ inp.placeholder='Describe tu empresa libremente...'; inp.value=''; inp.focus(); }
  const opt=el('chatOptions'); if(opt) opt.innerHTML='<span style="font-size:.8rem;color:#64748b">Escribe tu descripción abajo y presiona <strong>Enviar</strong> →</span>';
  const guia=el('guiaLibre'); if(guia) guia.style.display='block';
  actualizarGuiaCheck();
  if(inp) inp.addEventListener('input', actualizarGuiaCheck);
}
window.modoLibre = modoLibre;
window.toggleGuia = function(){
  const g=el('guiaLibre'); if(g) g.style.display = g.style.display==='none' ? 'block' : 'none';
}
window.usarEjemplo = function(n){
  ...
  const inp=el('freeInput'); if(inp){ inp.value=ejemplos[n]||ejemplos[0]; inp.focus(); actualizarGuiaCheck(); }
}
function actualizarGuiaCheck(){
  const inp=el('freeInput'); const box=el('guiaCheck'); if(!box) return;
  const v=(inp && inp.value || '').toLowerCase();
  let inf={}; try{ inf=nlp(v); }catch(e){}
  const checks=[
    {label:'Tamaño', ok: /\d+/.test(v) || /micro|pequeña|mediana|personas|equipo/.test(v)},
    {label:'Oferta / Sector', ok: !!inf.sector || /videojuego|software|obra|servicio|consult|bienes|suministro|hacemos|ofrecemos/.test(v)},
...
function loadDemo(){
  resetState();
  respuestas={empresa:'ConstruMetálicas S.A.S.',tam:'Pequeña',sector:'Obra',exp:'Ofertó',rup:'Trámite',secop:'Básico',equipo:'Parcial',objetivo:'Primer contrato'};
  score=65; idx=preguntas.length; updateProgress();
...
function finish(isDemo){
  ...
  const row=el('chatInputRow'); if(row) row.style.display='none';
  const guia=el('guiaLibre'); if(guia) guia.style.display='none';
  if(!isDemo){ addTyping(); setTimeout(()=>{removeTyping(); addMsg('Listo, ya tengo todo. <strong>Analizando con motor 4D + recomendación quirúrgica...</strong>');},380); }
...
function computeResult(){
  try{
    ...
    const t1=el('resTitle'); if(t1) t1.textContent=`Tu nivel: ${nivelInfo.nivel} • ${norm}/100`;
    const t2=el('resDesc'); if(t2) t2.innerHTML=`${intel.analisis}<br><br>${intel.causa}<br><em style="color:#e2e8f0">${intel.objTxt}</em><br><small style="color:#93c5fd">${intel.accion}</small>`;
    const sn=el('scoreNum'); if(sn) sn.textContent=norm;
    const gf=el('gaugeFill'); if(gf){ gf.style.width=norm+'%'; gf.style.background=nivelInfo.color; }
    const eb=el('resEyebrow'); if(eb) eb.textContent=`DIAGNÓSTICO IA 4D • ${respuestas.empresa||'Empresa'} • ${respuestas.sector||respuestas._inferSector||''} • v3`;
    const brechas=[];
    ...
    const bl=el('brechasList'); if(bl) bl.innerHTML=brechas.map(b=>`<li class="${b.includes('Radar')?'radar':''}">${b.includes('Radar')?b:'⚠ '+b}</li>`).join('');
    const ctxFull=el('radarFull');
    if(ctxFull && typeof Chart!=='undefined'){
      if(radarFullChart) { try{radarFullChart.destroy();}catch(e){} }
      radarFullChart=new Chart(ctxFull,{
        type:'radar',
        data:{ labels:['Habilitación','SECOP','Experiencia','Capacidad'], datasets:[{label:'Tu empresa', data:[dims.habilitacion, dims.secop, dims.experiencia, dims.capacidad], backgroundColor:'rgba(0,201,167,.18)', borderColor:'#00c9a7', pointBackgroundColor:'#0f1f3c'}, {label:'Meta 80%', data:[80,80,80,80], borderColor:'#e2e8f0', backgroundColor:'transparent', borderDash:[6,4], pointRadius:0}]},
        options:{ scales:{r:{min:0,max:100,ticks:{stepSize:20}, pointLabels:{font:{size:11}}}}, plugins:{legend:{position:'bottom'}}, animation:true }
      });
    }
    ...
    const rutas={
      'EXPLORADOR':[
        {f:'Fase 1 • 0-30 días',t:'Habilitación',d:`RUP + diagnóstico + SECOP básico. H ${dims.habilitacion}% → 65%.`},
      ...
      'INICIADO':[
        {f:'Fase 1 • 0-30 días',t:'Corrección',d:`SECOP intermedio + kit jurídico. S ${dims.secop}% → 75%.`},
      ...
      'EN DESARROLLO':[
        {f:'Fase 1 • 0-15 días',t:'Estrategia',d:`Consultoría Estado + mapa entidades. C ${dims.capacidad}% optimizada.`},
    ...
    const rs=el('rutaSteps'); if(rs) rs.innerHTML=rutas[nivelInfo.nivel].map(r=>`<div class="step"><span>${r.f}</span><h4>${r.t}</h4><p>${r.d}</p></div>`).join('');
    const registro={fecha:new Date().toISOString(), fechaStr:new Date().toLocaleString(), empresa:respuestas.empresa||'Anónima', nivel:nivelInfo.nivel, norm, score, dims};
    let hist=JSON.parse(localStorage.getItem('ideapro_hist_v3')||'[]'); hist.unshift(registro); hist=hist.slice(0,12);
    localStorage.setItem('ideapro_hist_v3',JSON.stringify(hist));
    localStorage.setItem('ideapro_last_v3',JSON.stringify({respuestas, nivel:nivelInfo.nivel, norm, dims, intel, reco}));
    renderHistorial(); updateEvoChart();
...
window.clearHistorial = function(){ localStorage.removeItem('ideapro_hist_v3'); localStorage.removeItem('ideapro_last_v3'); renderHistorial(); updateEvoChart(); }

function updateEvoChart(){
  try{
    if(typeof Chart==='undefined') return;
    const hist=JSON.parse(localStorage.getItem('ideapro_hist_v3')||'[]').slice().reverse();
    const ctx=el('evoChart'); if(!ctx) return;
    if(evoChart) { try{evoChart.destroy();}catch(e){} evoChart=null; }
    if(hist.length===0){
      evoChart=new Chart(ctx,{type:'line',data:{labels:['Sin datos'],datasets:[{label:'Madurez',data:[0]}]}, options:{plugins:{legend:{display:false}}, scales:{y:{min:0,max:100}}}});
      return;
    }
    evoChart=new Chart(ctx,{
      type:'line',
      data:{ labels:hist.map((h,i)=>`#${i+1} ${h.nivel.substring(0,3)}`), datasets:[{label:'Madurez /100', data:hist.map(h=>h.norm), borderColor:'#00c9a7', backgroundColor:'rgba(0,201,167,.12)', fill:true, tension:.35, pointBackgroundColor:'#0f1f3c'}]},
      options:{ responsive:true, plugins:{legend:{display:false}}, scales:{y:{min:0,max:100, ticks:{stepSize:20}}, x:{ticks:{font:{size:9}}}} }
    });
  }catch(e){ console.log('evo error',e); }
}

window.exportPerfil = function(tipo){
  const data=JSON.parse(localStorage.getItem('ideapro_last_v3')||'null');
  if(!data){ alert('Haz un diagnóstico primero'); return; }
  if(tipo==='json'){
    const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}); const url=URL.createObjectURL(blob);
    const a=document.createElement('a'); a.href=url; a.download=`perfil_${(data.respuestas.empresa||'ideapro').replace(/\s+/g,'_')}.json`; a.click();
  } else if(tipo==='pdf'){
    try{
      const {jsPDF}=window.jspdf;
      if(!jsPDF){ alert('jsPDF no cargó (¿sin internet?)'); return; }
      const doc=new jsPDF();
      doc.setFontSize(16); doc.text("IDEAPRO - Ficha Diagnóstico",14,18);
      doc.setFontSize(9); doc.text(`Empresa: ${data.respuestas.empresa||'-'}  |  Sector: ${data.respuestas.sector||'-'}  |  Fecha: ${new Date().toLocaleString()}`,14,26);
      doc.setFontSize(12); doc.text(`Nivel: ${data.nivel}  -  ${data.norm}/100`,14,34);
      doc.setFontSize(9); doc.text(`Habilitación: ${data.dims.habilitacion}%  SECOP: ${data.dims.secop}%  Experiencia: ${data.dims.experiencia}%  Capacidad: ${data.dims.capacidad}%`,14,40);
      doc.setFontSize(10); doc.text("Análisis IA:",14,48);
      const split=doc.splitTextToSize(data.intel?data.intel.analisis:"",180);
      doc.setFontSize(8); doc.text(split,14,54);
      ...
      doc.setFontSize(7); doc.text("Generado por Agente IDEAPRO v3 - Prototipo escalable",14,285);
...
async function loadServicios(){
  try{
    ...
    servicios=[
      {id:'diag360',nombre:'Diagnóstico 360 Mercado Público',tipo:'Diagnóstico',desc:'Radiografía completa: jurídica, financiera, técnica y comercial.',nivel:'Explorador',tag:'Puerta de entrada',precio:'Gratuito',duracion:'2 días',impacto:{habilitacion:30,secop:10,experiencia:15,capacidad:10}},
      {id:'rup',nombre:'Taller RUP Express',tipo:'Herramienta',desc:'Inscripción, renovación y actualización sin errores.',nivel:'Explorador',tag:'Habilitador',precio:'$ 280.000',duracion:'4 horas',impacto:{habilitacion:45,secop:5,experiencia:5,capacidad:5}},
      {id:'secop-bas',nombre:'Curso SECOP II Básico',tipo:'Capacitación',desc:'Cuenta, búsqueda inteligente, alertas y primera oferta.',nivel:'Explorador',tag:'Básico',precio:'$ 350.000',duracion:'8 horas',impacto:{habilitacion:10,secop:40,experiencia:10,capacidad:5}},
      {id:'secop-int',nombre:'SECOP II Intermedio + Pliegos Tipo',tipo:'Capacitación',desc:'Lectura crítica de pliegos y evitar rechazos.',nivel:'Iniciado',tag:'Intermedio',precio:'$ 580.000',duracion:'12 horas',impacto:{habilitacion:15,secop:35,experiencia:20,capacidad:10}},
      {id:'oferta',nombre:'Mentoría Perfilamiento de Oferta',tipo:'Mentoría',desc:'Ajusta portafolio para que el Estado sí te compre.',nivel:'Iniciado',tag:'Mentoría',precio:'$ 720.000',duracion:'3 sesiones',impacto:{habilitacion:25,secop:10,experiencia:20,capacidad:15}},
      {id:'juridico',nombre:'Kit Capacidad Jurídica y Financiera',tipo:'Herramienta',desc:'Checklists y modelos de indicadores.',nivel:'Iniciado',tag:'Kit',precio:'$ 320.000',duracion:'Autogestión',impacto:{habilitacion:20,secop:10,experiencia:10,capacidad:25}},
      {id:'simulador',nombre:'Simulador de Licitación Real',tipo:'Herramienta',desc:'Simula proceso completo con retroalimentación.',nivel:'En Desarrollo',tag:'Práctica',precio:'$ 450.000',duracion:'1 semana',impacto:{habilitacion:10,secop:25,experiencia:35,capacidad:15}},
      {id:'estrategia',nombre:'Consultoría Estrategia Comercial Estado',tipo:'Consultoría',desc:'Plan 90 días: entidades y pipeline.',nivel:'En Desarrollo',tag:'Estratégico',precio:'$ 2.800.000',duracion:'4 semanas',impacto:{habilitacion:15,secop:20,experiencia:25,capacidad:30}},
      {id:'financiero',nombre:'Mentoría Financiera para Licitar',tipo:'Mentoría',desc:'Cupo crédito, garantías y flujo.',nivel:'En Desarrollo',tag:'Financiero',precio:'$ 890.000',duracion:'2 sesiones',impacto:{habilitacion:10,secop:5,experiencia:10,capacidad:40}},
      {id:'pliegos',nombre:'Consultoría Pliegos y Objeciones',tipo:'Consultoría',desc:'Análisis pliegos complejos y subsanaciones.',nivel:'Avanzado',tag:'Avanzado',precio:'$ 1.600.000',duracion:'Por proceso',impacto:{habilitacion:10,secop:30,experiencia:30,capacidad:15}},
      {id:'alianzas',nombre:'Mentoría Alianzas y Consorcios',tipo:'Mentoría',desc:'Uniones temporales y consorcios ganadores.',nivel:'Avanzado',tag:'Escala',precio:'$ 1.100.000',duracion:'2 sesiones',impacto:{habilitacion:15,secop:15,experiencia:25,capacidad:35}},
      {id:'contenidos',nombre:'Kit Contenidos IDEAPRO',tipo:'Contenido',desc:'40 guías, plantillas y casos.',nivel:'Todos',tag:'Contenido',precio:'$ 180.000/año',duracion:'On-demand',impacto:{habilitacion:10,secop:15,experiencia:10,capacidad:10}}
    ];
  }
  renderCatalog();
}
function renderCatalog(){
  const inp=el('filterSvc'); const q=(inp && inp.value ||'').toLowerCase();
  const grid=el('svcGrid'); if(!grid) return;
...
window.addServicioDemo = function(){
  const nuevo={id:'demo'+Date.now(),nombre:'Servicio Demo Nuevo',tipo:'Mentoría',desc:'Ejemplo de cómo el negocio agrega servicios sin tocar código. Edita data/servicios.json',nivel:'Todos',tag:'Demo',precio:'$ 0',duracion:'1h',impacto:{habilitacion:10,secop:10,experiencia:10,capacidad:10}};
  servicios.unshift(nuevo); renderCatalog(); addMsg('Servicio demo agregado al catálogo vivo. Así escala IDEAPRO sin dev.','bot');
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  try{
    ...
    renderHistorial(); updateEvoChart();
    const bar=el('progressBar'); if(bar) bar.style.width='0%';
    const hero=el('btnHero'); if(hero) hero.onclick=(e)=>{ e.preventDefault(); const ag=el('agente'); if(ag) ag.scrollIntoView({behavior:'smooth'}); };
    const filt=el('filterSvc'); if(filt) filt.addEventListener('input', renderCatalog);
...
if(document.readyState!=='loading'){
  try{ loadServicios(); renderHistorial(); updateEvoChart(); }catch(e){}
}
